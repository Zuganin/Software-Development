...
public class YamlExportVisitorTests : IDisposable
{
    ...
    {
        // Arrange
        IExportVisitor visitor = new YamlExportVisitor(_outputDir);

        // Act
        visitor.Close();

        // Assert
        Assert.False(File.Exists(Path.Combine(_outputDir, "bankAccounts.yaml")));
        Assert.False(File.Exists(Path.Combine(_outputDir, "categories.yaml")));
        Assert.False(File.Exists(Path.Combine(_outputDir, "operations.yaml")));
    }

    [Fact]
    public void Close_ShouldCreateYamlFiles_WithData()
    {
        // Arrange
        IExportVisitor visitor = new YamlExportVisitor(_outputDir);
        ...
        // Assert
        var accountsPath = Path.Combine(_outputDir, "bankAccounts.yaml");
        var categoriesPath = Path.Combine(_outputDir, "categories.yaml");
        var operationsPath = Path.Combine(_outputDir, "operations.yaml");
    ...
    [Fact]
    public void Close_ShouldOverwriteExistingFiles_OnNewVisitorInstance()
    {
        // Arrange
        IExportVisitor visitor1 = new YamlExportVisitor(_outputDir);
        var account1 = new BankAccount("Первый", 500);
        visitor1.Visit(account1);
        visitor1.Close();

        IExportVisitor visitor2 = new YamlExportVisitor(_outputDir);
        var account2 = new BankAccount("Второй", 2000);
